  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      ctx.fillStyle = isDark
        ? "rgba(7, 8, 10, 0.95)" // Much darker fill to mask the "gray" transition
        : "rgba(255, 255, 255, 0.15)";

      // Note: Using a high opacity for dark mode initially clears the gray lag.
      // However, to keep the "trail" effect, we can use a lower opacity once stabilized.
      // For simplicity and immediate effect, let's use a dynamic opacity or just a darker constant.

      ctx.fillRect(0, 0, animationState.w, animationState.h);

      // Re-apply the semi-transparent trail if we want the star paths to show
      ctx.fillStyle = isDark
        ? "rgba(7, 8, 10, 0.18)"
          ...
          return (
            <div
              ...
              className={`${styles.card} ${
                visible.includes(idx) ? styles.visible : ""
              }`}
            >
